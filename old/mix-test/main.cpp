...
/*   Updated: 2022/02/09 11:43:08 by bledda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string phpPage(std::map<std::string, std::string> request_http, std::string file)
{
		std::string send;
		...
		HTTP_ACCEPT += request_http["Accept"];

		std::string HTTP_USER_AGENT = "HTTP_USER_AGENT=";
		HTTP_USER_AGENT += request_http["User-Agent"];

		std::string HTTP_ACCEPT_LANGUAGE= "HTTP_ACCEPT_LANGUAGE=";
		HTTP_ACCEPT_LANGUAGE += request_http["Accept-Language"];

		char **env = static_cast<char **>(calloc(sizeof(char *), 7));
		...
		env[5] = strdup(std::string("SCRIPT_FILENAME=" + file).c_str());
...
std::string httpPage(std::string file)
{
	std::string send;
...
std::string autoindex(std::string url)
{
	std::string send;
	...
	send += webserv::autoindex::autoindexHTML(std::string(url));
	return (send);
}
...
void gestion_client(t_sockaddr_in *server_interface, int server_socket)
{
	...
	while (1)
	{
		...
		chdir("/Users/bledda/Documents/webserv/");
		size_t s_php = URL.rfind(".php");
		size_t s_html = URL.rfind(".html");
		if (s_php + 4 == URL.size())
		{
			webserv::debug("Php detected");
			send = phpPage(value, "."+URL);
		}
		else if (s_html + 5 == URL.size())
		{
			webserv::debug("Html detected");
			send = httpPage("."+URL);
		}
		else if (URL.rfind("favicon.ico") == -1)
		{
			webserv::debug("Folder detected");
			send = autoindex("."+URL);
		}
